...
const Home = () => {
    ...
    useEffect(()=>{
        ...
        const getTasks= ()=>{
            fetch('https://assets.breatheco.de/apis/fake/todos/user/carlottaj', {
            method: "GET",
            headers: {
...
useEffect(()=>{
    ...
    const html =tasks.map((task, i)=>{
     ...
     return <li class="list-group-item"    key={i}>{task.label};

        <button className="btnPlus"  onClick={()=>resetButton(i)}value={tasks}> X </button>
        </li>
    })
    return (
